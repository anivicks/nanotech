...
def combinedGate(gate1, gate2):
    gate1 = gate1
    gate2 = gate2
    combGateArray = []

    for a in range(len(gate1)):
        combGateArr = []
        for b in range(len(gate1[a])):
            combGate = np.bitwise_or(int(gate1[a][b]), int(gate2[a][b]))
            # if combGate == 0:
            #     combGate = 1
            # else:
            #     combGate = 0
...
def forLastFour():
    ...
    for x in range(5):
        ...
        for y in range(4):
            num1FourArr.append(firstNum[len(firstNum) - 1 - y])
            num2FourArr.append(secondNum[len(secondNum) - 1 - y])
    ...
    # f = open("f1.txt", "w")
    # for x in range(5):
    #     f.write(eightDigitsArray[x] + ' ' + num1FourArray[x] + ' ' + num2FourArray[x] + '\n')
    # f.close()
...
print(checkRelation(combinedGate(andGate(), orGate())))